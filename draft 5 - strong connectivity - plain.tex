...
\documentclass[leqno,english]{article}
...
The firefighter problem was introduced by Hartnell and it is based
on the following one player combinatorial game: let $G=\left(V,E\right)$
be a graph, and $S\subseteq V$, finite subset of the burning vertices
at time 1. At every time step $t\in\mathbb{N}$ , the player choose
$\mathsf{f}\left(t\right)$ non burning vertices and protect them.
then, at time $t+\frac{1}{3}$ all the unprotected neighbors of the
fire will lit.

For every $\mathsf{f}:\mathbb{N}\rightarrow\mathbb{N}$, look at the
family of strategies $\mathcal{F}\left(\mathsf{f}\right)$ such that
the total number of protected vertices until time $t$ is less or
equals $\mathsf{f}\left(t\right)$\todo{O: Either "at most" or rounded. Also not consistent with notation later, see remark in the theorem.}. We say that $\mathsf{f}$ \emph{contain} the fire if for all finite $S$ there is a strategy in $\mathcal{F}\left(\mathsf{f}\right)$
such that the fire is bounded.

Let $G$ be a graph, one can ask for optimal $\mathsf{f}$ in some
sense that depends on the growth of $G$, for example, for $\mathbb{Z}\boxempty\mathbb{Z}$
any $\mathsf{f}\left(t\right)=ct$ with $\frac{3}{2}<c$ contain the
fire and $\mathsf{f}=\frac{3}{2}t$ is not enough \cite{key-1}\todo{You mean that for every such f the fire could be contained. f is not a strategy.}\todo{A: We define the term "contain" to be an attribute of a function}. The
optimal $\mathsf{f}$ is known for a few graphs and there are bounds
...
This paper generalize the Cartesian result by proving that $\frac{3}{2}ht$
...
For two graphs $G,H$ denote by $G\boxempty H$ the Cartesian product of $G,H$ and denote by $G\boxtimes H$ the strong Cartesian product of $G,H$.
\begin{thm}
\label{thm:1}For every $h\in\mathbb{N}$, let $G_{h}=\mathbb{Z}\boxempty\mathbb{Z}\times L_{h}$.
Let $\frac{3}{2}<c$ then $\mathsf{f}\left(t\right)=cht$ contain
the fire, and $\mathsf{f}=\frac{3}{2}ht$ are not enough.
\end{thm}

\begin{thm}
\label{cor:2}For every $h\in\mathbb{N}$, let $G_{h}=\mathbb{Z}\boxtimes\mathbb{Z}\times L_{h}$.
Let $3<c$ then $\mathsf{f}\left(t\right)=cht$ contain the fire,
and $\mathsf{f}=3ht$ are not enough.
\end{thm}


\section{Preliminaries}

Due to time ambiguity in the firefighter problem description, we
define the time line of the game:
\begin{center}
\begin{tabular}{|c|c|}
\hline 
...
\hline 
\hline 
0 & The grid created\tabularnewline
\hline 
$\frac{1}{3}$ & The initial fire created \tabularnewline
\hline 
$n-\frac{1}{3}$ & Protected vertices are selected\tabularnewline
\hline 
$n$ & Nothing\tabularnewline
\hline 
$n+\frac{1}{3}$ & The fire spreads to adjacent vertices\tabularnewline
\hline 
\end{tabular}
\par\end{center}

Most of the arguments will be local and patched together. We begin
by setting up the infrastructure for a local system.

The four axis aligned directions, denoted by $e_{0}=\left(0,1\right)=-e_{2},e_{1}=\left(1,0\right)=-e_{3}$.

Throughout, we follow the convention that addition in subscript indices
is taken modulo 4.

Fix $h\in\mathbb{N}$ and let $G_{h}=\mathbb{Z}\boxtimes\mathbb{Z}\times L_{h}$.
The vertices of $G_{h}$ are $V:=\mathbb{Z}\times\mathbb{Z}\times\left[h\right]$,
therefore identify the vertices with $\mathbb{Z}\times\mathbb{Z}\times\left[h\right]\subseteq\mathbb{Z}^{3}$.
\begin{defn}
...
\begin{cor}
\label{cor:1}The z coordinate give us that $L_{i}^{k}\left(t\right),L_{i}^{\ell}\left(t\right)$
are disjoint for all $k\ne\ell$.

The x coordinate of points in $L_{1}$ is $r_{1}^{k}\left(t\right)$, the x
coordinate of $L_{0}$ is strictly less than $r_{1}^{k}\left(t\right)$
so that $L_{1}^{k}\left(t\right)\cap L_{0}^{k}\left(t\right)=\varnothing$.
The same holds for all $i\ne j$. 
\end{cor}

\begin{defn}
Denote by $B\left(t\right),F\left(t\right)\subseteq\mathbb{Z}^{2}\times\left[h\right]$
the burning vertices and the firefighters at time $t$.
\end{defn}
...
\end{rem*}
\begin{cor}
Let $t<t^{\prime}$, by induction: $\mathsf{F}_{i}^{k}\left(t^{\prime}\right)\cap\mathsf{F}_{i}^{k}\left(t\right)=\varnothing$.
\corref{1} shows the disjointness of $L_{i}^{k}\left(t\right),L_{j}^{\ell}\left(t\right)$
...
\]
%Arye: maybe writing the i,k summation together, like that:
...
To obtain some properties of the fire and the distance we need a
``frontal structure''.
\begin{defn}
...
all $k\leq m<\ell$ conclude that the m-th level is pulled.
\end{cor}
...
k-th level being pulled by the $\ell$-th level (take $\ell$ to be
minimal) , by \corref{4} we have $r_{i}^{\ell-1}\left(t\right)=r_{i}^{\ell}\left(t\right)-1$
...
\cite{key-1} one can deduce that in this model 
\[
...
also $\Phi_{1}^{k}\left(t\right)^{+}\cap L_{0}^{k}\left(t+1\right)=\varnothing$
these two gives:
\[
...
\bibitem{key-1} Ohad N. Feldheim, Rani Hod, 3/2 Firefighters are
not enough.
